...
onMounted(() => {
  if (router.query.index) {
    orderInfo.value = orderList[router.query.index];
  }
});
</script>
...
<template>
  <van-nav-bar
    :title="router.query.index >= 0 ? '编辑订单' : '新增订单'"
    left-text="返回"
    left-arrow
  ...
  <van-form @submit="onSubmit" class="form">
    <van-cell-group inset>
      ...
      <van-field
        v-model="orderInfo.person"
        center
        clearable
        label="收货人"
        placeholder="请输入/选择收货人"
        input-align="right"
  ...
  >
    <van-picker
      ...
      @confirm="handleGoodsPick"
      @cancel="goodsPickerShow = false"
    />
  </van-popup>

  <van-popup
    v-model:show="personPickerShow"
    position="bottom"
    :style="{ height: '50%' }"
  >
    <van-picker
      ...
      @confirm="handlePersonPick"
      @cancel="personPickerShow = false"
    />
  </van-popup>
